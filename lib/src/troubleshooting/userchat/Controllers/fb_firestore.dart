...
class FBCloudStore {
  ...
  Future<List<String>> saveUserDataToFirebaseDatabase(userEmail,userId,userName,userIntro,downloadUrl) async {
    try {
      ...
      final QuerySnapshot result = await FirebaseFirestore.instance.collection('chats').where('userId', isEqualTo: prefs.get('userId')).get();
      ...
      if (documents.length == 0) {
        ...
        await FirebaseFirestore.instance.collection('chats').doc(myID).update({
          'email':userEmail,
          'name':userName,
          'intro':userIntro,
          'userImageUrl':downloadUrl,
  ...
  Future<void> updateMyChatListValues(String documentID,String chatID,bool isInRoom) async{
    ...
    final DocumentReference result = FirebaseFirestore.instance.collection('chats').doc(documentID).collection('chatlist').doc(chatID);
  ...
  Future<void> updateUserToken(userID, token) async {
    await FirebaseFirestore.instance.collection('chats').doc(userID).update({
      'FCMToken':token,
    });
  }
